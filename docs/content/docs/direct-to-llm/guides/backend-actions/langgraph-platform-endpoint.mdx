...
<Steps>
    ...
    <TailoredContent id="hosting">
        <TailoredContentOption
            ...
            icon={<FaCloud />}
        >
            If you followed the [Copilot Cloud Quickstart](/quickstart) and opted to use Copilot Cloud,
            ...
            <Accordions className="my-4">
                <Accordion title="Haven't set up Copilot Cloud yet? Click here!" >
...
<Callout type="info">
  ...
  Guide](/langgraph/shared/guides/langgraph-platform-authentication).
</Callout>
...
4. **Authentication issues**: If you're having trouble with authentication, check that you're passing the `authorization` token in the `properties` prop and that your agent is properly validating it. See the [authentication guide](/langgraph/shared/guides/langgraph-platform-authentication) for details.
