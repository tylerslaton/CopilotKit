...
<Accordions>
  <Accordion title="Changing where/when the action is executed">
    ...
    - `enabled`: The action is enabled and can be used by the LLM or [CoAgent](/langgraph).
    - `disabled`: The action is disabled and cannot be used by the LLM or [CoAgent](/langgraph).
    - `remote`: When using a [CoAgent](/langgraph), the action is only passed to the CoAgent, but not the LLM.
